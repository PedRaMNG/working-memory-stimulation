% you have to run and plot with these variables
% then change the settings and run and plot to get the rest of the figures.
% and put everything together in adobe illustrator or visio

% xx = 1
% num_learned_patterns = length(learned_patterns)*xx;
% when j == length(learned_patterns) add one to xx


images = model.images;
V_line_E = model.V_line_E;
T_Iapp = model.T_Iapp;
    
num_learn_patterns = length(params.learn_order);
pattern_shift = num_learn_patterns*1; % For Enhance change this to 1 and 2
% pattern_shift = num_learn_patterns*2;

learned_patterns = unique(params.learn_order);
num_learned_patterns = length(learned_patterns);


% learned_patterns = unique(params.test_order); % Use this for Augmentation to check the test patterns
% num_learned_patterns = length(learned_patterns);

test_patterns = params.test_order;

spike_count = zeros(params.quantity_neurons_E, num_learned_patterns);

for j = 1:num_learned_patterns
    pattern_id = find(test_patterns == learned_patterns(j));
    pattern_id = pattern_id(1); 
    for i = 1:params.quantity_neurons_E
        be = T_Iapp(pattern_shift + pattern_id,1) + 1100;
        en = T_Iapp(pattern_shift + pattern_id,2) + params.impact_astro;
        spike_count(i,j) = sum(V_line_E(i, be:en) > params.neuron_fired_thr - 1);
    end
end

spike_images = reshape(spike_count, params.mneuro_E, params.nneuro_E, []);
...
memory_performance = performance; clear performance
