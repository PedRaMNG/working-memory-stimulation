...
function [I_signals, full_timeline, ...
    timeline_signal_id, timeline_signal_id_movie] = ...
      make_experiment(images)
    ...
    % Check if simPattern is 3, and adjust the test_timeline accordingly
    if params.simPattern == 3
    % Repeat the test_order twice with a delay of 3 * params.test_impulse_shift
        test_order = [test_order, test_order];
        test_timeline  = make_timeline(params.test_start_time, ...
                                        params.test_impulse_duration, ...
                                        params.test_impulse_shift, ...
                                        length(test_order));
        
        % Add delay of 3 * params.test_impulse_shift between the two repeated sequences
        for i = (length(test_order) / 2 + 1):length(test_order)
            test_timeline(i, 1) = test_timeline(i, 1) + params.delayTests;
            test_timeline(i, 2) = test_timeline(i, 2) + params.delayTests;
        end
    else
        % Use the original test_timeline if simPattern is not 3
        test_timeline  = make_timeline(params.test_start_time, ...
                                        params.test_impulse_duration, ...
                                        params.test_impulse_shift, ...
                                        length(test_order));
    ...
    full_timeline = uint32(full_timeline); 
    %----------------------------------------------------------------------
