function I_poisson_noise = make_poisson_noise()
    ...
    T_poisson                 = poissrnd(T_step, params.quantity_neurons_E, params.poisson_n_impulses);
    amplitude                 = (rand(params.quantity_neurons_E, params.poisson_n_impulses) - 0.5) * 2;
    amplitude                 = amplitude * params.poisson_amplitude;
    T_poisson(:,1)            = fix(params.poisson_impulse_initphase * rand(params.quantity_neurons_E, 1));
    T_poisson(T_poisson == 0) = 1;
    T_poisson                 = cumsum(T_poisson, 2);
    
    I_poisson_noise           = zeros(params.quantity_neurons_E, max(T_poisson, [], 'all'));
